    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<string> ChangePasswordAsync(int id, string oldPassword, string newPassword)
        {
            try
            {
                ...
                if (!BCrypt.Net.BCrypt.Verify(oldPassword, user.PasswordHash)) return "Invalid old password.";
            ...
            catch (Exception ex) { return $"An error occurred while changing the password: {ex.Message}"; }
        }
        public async Task<string> ChangeNameAsync(int id, string name)
        {
            ...
            {
                var user = await _context.Users.FirstOrDefaultAsync(u => u.Id == id);
                if (user == null) return "An error occurred while changing the name";
                user.Name = name;
                await _context.SaveChangesAsync();
                return user.Name;
            }
        ...
        public async Task<string> ChangeEmailAsync(int id, string email, string code)
        {
            try
            {
                var user = await _context.Users.FirstOrDefaultAsync(u => u.Id == id);
                if (user == null) return "User not found";
                var chickCode = await _context.Otps.Where(o => o.Email == user.Email).OrderByDescending(x => x.CreatedAt).FirstOrDefaultAsync();
                if (chickCode == null) return "Not found OTP code";
                if (chickCode.Code != code) return "Invalid OTP code";
                user.Email = email;
                await _context.SaveChangesAsync();
                return user.Email;
            }
        ...
        {
            var user = await _context.Users.Include(p => p.Addresses).FirstOrDefaultAsync(u => u.Id == userId);
            if (user == null) return "User not found";
        ...
        public async Task<string> DeleteAddressAsync(int addressId, int userId)
        {
            var user = await _context.Users.Include(p => p.Addresses).FirstOrDefaultAsync(u => u.Id == userId);
            if (user == null) return "User not found";
            var addressToDelete = user.Addresses.FirstOrDefault(u => u.Id == addressId);
            if (addressToDelete == null) return "Address not found";
        ...
        public async Task<string> SetDefaultAddressAsync(int addressId, int userId)
        {
            ...
            if (!userAddresses.Any()) return "No addresses found for this user";
            var currentDefault = userAddresses.FirstOrDefault(u => u.IsDefault);
            if (currentDefault != null) currentDefault.IsDefault = false;
            var addressToSetDefault = userAddresses.FirstOrDefault(u => u.Id == addressId);
            if (addressToSetDefault == null) return "Address not found";
